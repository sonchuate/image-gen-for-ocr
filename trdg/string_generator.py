...
def get_string(s: str, num_word: int = 10) -> str:
    list_w = [sub_s for sub_s in s.split(' ') if len(sub_s) > 0]
    return " ".join(list_w[:num_word])
...
def create_strings_from_file(filename: str, count: int) -> List[str]:
    ...
    with open(filename, "r", encoding="utf8") as f:
        lines = [get_string(l, rnd.randint(5, 20)) for l in f.read().splitlines() if len(l) > 0]
        if len(lines) == 0:
